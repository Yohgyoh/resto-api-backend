...
const MONGO_URI = process.env.MONGO_URI; // Pake nama variabel lo
const httpServer = http.createServer(app);
const io = new Server(httpServer, { /* ... konfig cors lo ... */ });
io.on('connection', (socket) => { /* ... listener socket lo ... */ });

// --- BAGIAN YANG DIUBAH ---

// 1. LANGSUNG JALANIN SERVER HTTP-NYA DULU
httpServer.listen(port, '0.0.0.0', () => {
  console.log(`App (plus Socket.IO) dengerin di port ${port}`);
});

// 2. KONEKSI KE DB DIJALANIN SECARA TERPISAH
mongoose
  .connect(MONGO_URI) // <-- Pake MONGO_URI
